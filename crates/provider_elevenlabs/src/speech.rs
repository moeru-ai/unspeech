...
pub async fn handle(options: ProcessedSpeechOptions, client: Client, token: &str) -> Result<(HeaderMap, Bytes), AppError> {
  let voice_settings: ElevenLabsSpeechVoiceSettings = match options.extra.get("voice_settings") {
    Some(v) => ElevenLabsSpeechVoiceSettings { speed: options.speed, ..serde_json::from_value::<ElevenLabsSpeechVoiceSettings>(v.clone())? },
  ...
  let res = client.post(url.as_str()).header("xi-api-key", token).json(&body).send().await?;
